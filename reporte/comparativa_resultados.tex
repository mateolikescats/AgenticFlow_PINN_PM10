...
\begin{tabularx}{\textwidth}{lXX} % 'X' estira la columna y auto-ajusta el texto
\toprule
\textbf{Métrica de Evaluación} & \textbf{Línea Base (Actual)} & \textbf{Modelo iPINN Propuesto (Optimizado)} \\
\midrule
Loss Total PINN & $0.454$ & \textit{Por evaluar} \\
Estabilidad Termodinámica & Inestable ($N^2 \le 0$ local) & Estricta ($N^2 > 0$ global) \\
Conservación de la Masa ($PM_{2.5}$) & No conservativo (fuga vertical) & Conservativo (flujo retenido) \\
Modelo de Fuente $S(x, z, t)$ & Hardcodeado ($S = 0$) & Parametrizado Dinámico Inverso (iPINN) \\
Discretización de Laderas & Caja Rectangular Plana & Cañón en Tazón mediante Mapeo Sigma \\
Densidad de Colocación & Uniforme (Mapeo plano) & Importance Sampling (Gaussiano en base) \\
Estrategia de Optimización & Adam (50 Épocas) & Híbrido Adam (300) + L-BFGS (Curvatura) \\
\bottomrule
\end{tabularx}
\end{table}

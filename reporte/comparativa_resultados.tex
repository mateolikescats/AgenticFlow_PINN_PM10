...
\begin{tabular}{lcc}
\toprule
\textbf{Métrica de Evaluación} & \textbf{Línea Base (Antes)} & \textbf{iPINN Activa (Propuesta 1 - Actual)} \\
\midrule
Loss Total PINN & $0.454$ & $0.462$ (Estable con fuente acoplada) \\
Conformidad Física (Flotabilidad) & Inestable ($N^2 \le 0$ local) & Parcial ($N^2 > 0$ inducido por softplus) \\
Conservación de la Masa ($PM_{2.5}$) & No conservativo (fuga vertical) & Conservativo ($S \ge 0.0$ garantizado) \\
Modelo de Fuente $S(x, z, t)$ & Fijo ($S = 0$) & Activo (6ª Red Lux + softplus) \\
Resolución de Bordes (Laderas) & Caja Rectangular Plana & Caja Rectangular Plana (Base) \\
Optimizador Utilizado & Adam (50 Épocas) & Adam (50 Épocas) \\
\bottomrule
\end{tabular}
\end{table}

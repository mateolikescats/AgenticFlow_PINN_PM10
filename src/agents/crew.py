...
reaction_validator = Agent(
    ...
    goal="Validar que los campos físicos de velocidad y temperatura de la PINN sean físicamente consistentes para las predicciones de las últimas 96 horas.",
    backstory=(
        "Eres un analista termodinámico exigente. "
        "Revisas que la inversión térmica atrapante funcione físicamente en el periodo de las últimas 96 horas de predicciones. "
...
forensic_investigator = Agent(
    ...
    backstory=(
        ...
        "Utilizas herramientas de clustering GMM para identificar nubes de contaminación a partir de las predicciones de las últimas 96 horas "
...
policy_advisor = Agent(
    ...
    goal="Generar alertas tempranas y políticas dinámicas basadas en la severidad de la inversión y la ubicación de las emisiones de las últimas 96 horas.",
    backstory=(
        "Eres un consultor de política ambiental y salud pública de Medellín. "
        "Lees los reportes forenses y las consistencias físicas de las últimas 96 horas, y formulas directrices estrictas de emergencia "
...
latex_reporter = Agent(
    ...
    goal="Consolidar todos los reportes agénticos basados en las predicciones de las últimas 96 horas en un documento standalone reporte_forense.tex de calidad de publicación.",
    backstory=(
        ...
        "sobre el estado atmosférico y de emisiones de las últimas 96 horas, con tablas de métricas físicas, atribuciones espaciales y planes reguladores dinámicos."
    ),
    verbose=True,
    allow_delegation=False,
...
task_validate_thermodynamics = Task(
    description=(
        "1. Analiza los resultados del entrenamiento físico y las predicciones de las últimas 96 horas de la iPINN. \n"
        ...
        "y si la velocidad vertical está correctamente atenuada cerca de las laderas sólidas bajo las condiciones de las últimas 96 horas.\n"
    ...
    expected_output="Un dictamen termodinámico formal detallando la estabilidad física, el confinamiento de contaminantes y las métricas PVI bajo las predicciones de las últimas 96 horas.",
    agent=reaction_validator
)

task_clustering_attribution = Task(
    description=(
        "1. Ejecuta la 'Spatiotemporal GMM Clustering Tool' para encontrar 2 cúmulos principales de contaminación a partir de las predicciones físicas de las últimas 96 horas.\n"
...
task_policy_advice = Task(
    description=(
        "1. Lee el dictamen termodinámico y el informe de atribución geoespacial correspondientes a las predicciones de las últimas 96 horas.\n"
        ...
        "y el estado estable de la inversión térmica durante estas últimas 96 horas."
    ),
...
task_generate_latex_report = Task(
    description=(
        "1. Recopila el dictamen termodinámico, el informe forense geoespacial y el plan de políticas de los otros agentes para el periodo de las últimas 96 horas.\n"
        "2. Diseña y redacta un reporte standalone completo en código LaTeX, que incluya:\\documentclass, \\begin{document}, "
        "un título formal de 'Reporte Forense Ambiental y Atribución Atmosférica (Últimas 96 Horas)', y secciones estructuradas.\n"
